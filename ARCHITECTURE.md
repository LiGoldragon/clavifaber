...
`publication.datom` file alongside the Yggdrasil projection and the
...
The CLI is **Datom-only**: one positional Datom record per invocation.
...
| `YggdrasilKeypairSetup` | Generate the per-host Yggdrasil keypair file (mode 0600); return a typed `YggdrasilProjection`. |
| `PublicKeyPublicationWriting` | Read sshd's `ssh.pub` + (optional) yggdrasil keypair + (optional) wifi cert file; assemble and atomically write `publication.datom`. |
...
- **Cluster-side consumers.** `publication.datom` lands on disk; whoever
...
```mermaid
flowchart TB
    cli["clavifaber CLI<br/>(Datom-only positional record)"] --> request["ClaviFaberRequest<br/>(generated Datom enum)"]
...
| `publication.datom` carries a typed `YggdrasilProjection` record. | `tests/publication_writing.rs::public_key_publication_writing_assembles_typed_record_atomically`. |
| `publication.datom` carries a typed `WifiClientCertificate` record. | Same test asserts on the typed wrapper. |
...
| `publication.datom` is mode 0644 (publicly readable). | Same test asserts `stat -c %a` = 644. |
...
| `publication.datom` mode 0644. | `tests/publication_writing.rs::public_key_publication_writing_assembles_typed_record_atomically`. |
...
├── main.rs                 — #[tokio::main] CLI entry; one Datom record in, one Datom record out
...
└── request_surface.rs          — Datom round-trip witnesses
